...
<div @class([$wrapperClass])>
    ...
    <input
        ...
        @required($required)
        {{ $attributes->except('id')->merge(['id' => $attributes->get('id', $name)])->class(['mt-2 block w-full rounded-xl border bg-white px-3.5 py-2.5 text-cocoa-950 shadow-sm transition placeholder:text-cocoa-500/60', 'border-red-300' => $errors->has($name), 'border-cream-300 hover:border-cocoa-500/50' => ! $errors->has($name)]) }}
    >
