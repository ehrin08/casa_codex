    ...
    <body class="min-h-screen bg-zinc-50 text-zinc-900 antialiased">
        @php
            $navigation = [
                ...
                ['label' => 'Management', 'route' => 'management.index'],
                ['label' => 'Therapist', 'route' => 'therapist.index'],
                ['label' => 'Customer', 'route' => 'customer.index'],
            ];
        @endphp
        ...
        <div class="flex min-h-screen flex-col">
                    ...
                    <nav class="flex flex-wrap gap-2 text-sm font-medium">
                        @foreach ($navigation as $item)
                            <a
                                ...
                                class="rounded-md px-3 py-2 transition {{ request()->routeIs($item['route']) ? 'bg-emerald-700 text-white' : 'text-zinc-600 hover:bg-zinc-100 hover:text-zinc-950' }}"
                            >
                                {{ $item['label'] }}
                            </a>
                        @endforeach
            ...
            <main class="flex-1">
                <section class="border-b border-zinc-200 bg-white">
                    <div class="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
                        <p class="text-sm font-semibold uppercase tracking-wide text-emerald-700">Sprint 1 foundation</p>
            ...
            <footer class="border-t border-zinc-200 bg-white">
                <div class="mx-auto flex w-full max-w-6xl flex-col gap-2 px-4 py-6 text-sm text-zinc-500 sm:px-6 md:flex-row md:items-center md:justify-between lg:px-8">
                    ...
                    <span>Project setup and system foundation</span>
                </div>
            </footer>
        </div>
    </body>
